...
export const RentBikeQuery = () => {
  ...
  useEffect(() => {
    if (location) {
      fetchApi("/v2/Bike/Station/NearBy", {
        $top: 10,
        $fomat: "JSON",
        $spatialFilter: `nearby(${location.latitude},${location.longtitude},500)`,
      }).then((response) => {
            ...
            <div className="bg-purple-custom	row-span-3 rounded-3xl p-8">
              <div className="flex mb-4">
                <button
                  ...
                  className="bg-tertiary px-6 py-2 rounded-3xl text-white font-bold  w-28"
                >
                  離我最近
                </button>
                <button className="bg-gray-50 px-6 py-2 rounded-3xl text-gray-700 font-bold ml-4 w-32">
                  下拉式選單
                </button>
              </div>
              <SearchBar />
            </div>
            <div className="bg-gray-100  row-span-9  rounded-3xl"></div>
